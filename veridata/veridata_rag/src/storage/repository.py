...
async def get_tenant_languages(tenant_id: UUID) -> Optional[str]:
    async for session in get_session():
        try:
            result = await session.execute(select(Tenant.preferred_languages).where(Tenant.id == tenant_id))
    ...
    async for session in get_session():
        try:
            ...
            await session.execute(stmt, {
                ...
                "filename": filename,
                "content": content,
                "embedding": str(embedding) # pgvector usually takes list but let's see if sqlalchemy handles list->vector auto.
    ...
    async for session in get_session():
        try:
            ...
            result = await session.execute(stmt, {
                "embedding": str(query_embedding), # Pass as string representation for vector cast
                "tenant_id": tenant_id,
                "limit": limit,
